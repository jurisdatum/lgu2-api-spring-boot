...
<xsl:variable name="hideLimitedEUdata" as="xs:boolean" select="true()" />
<xsl:variable name="isDeIndexedEUDocument" as="xs:boolean" select="exists(/leg:Legislation/ukm:Metadata//ukm:DocumentCurrentStatus/ukm:UKAmended[@Value='false']) or 
	exists(/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://www.legislation.gov.uk/def/legislation/DeIndexedEUDocument']/@title) or 
	(/leg:Legislation/ukm:Metadata/ukm:EUMetadata/ukm:DocumentClassification/ukm:DocumentMainType/@Value = 'EuropeanUnionDirective')"/>
...
<xsl:variable name="nstSelectedSection" as="element()?"
	select="(/leg:Legislation/(leg:Primary | leg:Secondary | leg:EURetained)/(leg:Body | leg:EUBody | leg:Schedules | leg:Attachments)//*[@id != '' and @DocumentURI = $strCurrentURIs])[1]" />
...
<xsl:variable name="createdTypes" as="xs:string*"
	...
	<!-- NOTE this variable is for content welsh NOT for page furniture welsh-->
	<xsl:variable name="language" select="if (/leg:Legislation/@xml:lang) then /leg:Legislation/@xml:lang else 'en'"/>
...
<xsl:variable name="tso:legTypeMap" as="element()+">
  ...
  <tso:legType schemaType="UnitedKingdomLocalAct" abbrev="ukla" class="primary" category="Act"
	...
	<tso:legType schemaType="UnitedKingdomPrivateOrPersonalAct" abbrev="ukppa" class="primary" category="Act"
		...
		start="1801" complete="1946" revised="false" />	
  ...
  <tso:legType schemaType="IrelandAct" abbrev="aip" class="primary" category="Act"
	...
	<tso:legType schemaType="GreatBritainLocalAct" abbrev="gbla" class="primary" category="Act"
		...
		start="1797" complete="1801" revised="false" />
  ...
  <tso:legType schemaType="WelshStatutoryInstrument" class="secondary" category="Instrument" abbrev="wsi"
  	em="Explanatory Memorandum" singular="Welsh Statutory Instrument" pn="Policy Note" plural="{leg:TranslateText('Welsh Statutory Instruments')}"
  	start="1999" complete="1999" revised="true" />
  ...
  <tso:legType schemaType="WelshDraftStatutoryInstrument" class="draft" category="Instrument" abbrev="wdsi"
  	...
  	start="2026" complete="2026" legType="WelshStatutoryInstrument" />
-->	
...
<xsl:variable name="leg:welshType" as="xs:string+">
	<xsl:sequence select="('WelshStatutoryInstrument', 'WelshDraftStatutoryInstrument', 'WelshAssemblyMeasure','WelshNationalAssemblyAct','WelshParliamentAct')"/>
</xsl:variable>
...
<xsl:function name="tso:GetEffectingTypes" as="element(tso:legType)+">
	<xsl:sequence select="$tso:legTypeMap[not(@class = ('draft','IA', 'Bill')) and (@start >= 2002 or @complete >= 2002 or not(@end))]"/>
</xsl:function>
...
<xsl:function name="tso:getTitleFromEffects" as="xs:string">
	<xsl:param name="title" />
	<xsl:param name="type" as="xs:string"/>
	<xsl:choose>
		<xsl:when test="$type = $leg:welshType and $TranslateLang='cy'">
			<xsl:value-of select="($title[@xml:lang='cy'][1], $title)[1]"/>
		</xsl:when>
		<xsl:when test="$type = $leg:welshType and $TranslateLang='en'">
			<xsl:value-of select="$title[not(@xml:lang='cy')][1]"/>
		</xsl:when>
		<xsl:otherwise>
	...
	<div id="checkboxes">
		<div class="searchCol2">
			...
			<xsl:if test="$numberOfItems != 0">
				...
				<div id="legChoicesColLeft" class="typeCheckBoxCol" style="width:400px">
					<xsl:copy-of select="$dropListItems/*[position() &lt; xs:integer(ceiling($numberOfItems div 2))+1]"/>
				</div>
				<div id="legChoicesColRight" class="typeCheckBoxCol" style="width:300px">
...
<xsl:function name="tso:GetNumberForLegislation" as="xs:string">
	...
	<xsl:value-of>
		<xsl:choose>
			<xsl:when test="$type = ('UnitedKingdomPublicGeneralAct', 'GreatBritainAct', 'UnitedKingdomPrivateOrPersonalAct', 'GreatBritainPrivateOrPersonalAct', 'EnglandAct', 'IrelandAct', 'NorthernIrelandAct', 'ScottishOldAct')">
				<xsl:text>c. </xsl:text>
				<xsl:value-of select="$number" />
			</xsl:when>
			<xsl:when test="$type = ('UnitedKingdomLocalAct', 'UnitedKingdomLocalActRevised', 'GreatBritainLocalAct')">
				<xsl:text>c. </xsl:text>
...
<xsl:function name="leg:TranslateText" as="xs:string">
	...
	<xsl:function name="leg:TranslateExtent" as="xs:string">
		<xsl:param name="id" as="xs:string"/>
		<xsl:value-of select="leg:TranslateTextToLang($id, $language)" />
	</xsl:function>
	
	<xsl:function name="leg:TranslateMessage" as="xs:string">
		<xsl:param name="id" as="xs:string"/>
		<xsl:value-of select="leg:TranslateTextToLang($id, $language)" />
	</xsl:function>
