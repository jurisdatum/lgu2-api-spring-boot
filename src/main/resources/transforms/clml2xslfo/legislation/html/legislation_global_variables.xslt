...
exclude-result-prefixes="leg ukm math msxsl dc dct ukm fo xsl svg xhtml tso xs err">
	...
	<xsl:variable 	name="g_strDocumentLang" as="xs:string?" select="$g_ndsMetadata/dc:language"/>
	...
	<xsl:variable 	name="g_hasVersionEn"  as="element()*"
					select="$g_ndsMetadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion'][@hreflang = 'en' or empty(@hreflang)]"/>	
	<xsl:variable 	name="g_hasVersionCy"  as="element()*"
					select="$g_ndsMetadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion'][@hreflang = 'cy']"/>
	<xsl:variable 	name="g_currentEn"  as="element()*"
					select="$g_hasVersionEn[@title='current']"/>
	<xsl:variable 	name="g_currentCy"  as="element()*"
					select="$g_hasVersionCy[@title='current']"/>				
	...
	<xsl:variable 	name="g_strEOURI" as="xs:string?" 
					select="$g_ndsMetadata/atom:link[@rel='http://www.legislation.gov.uk/def/navigation/earlier-orders' and @title='earlier orders']/@href"/>
