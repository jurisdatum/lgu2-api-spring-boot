...
xmlns="http://www.w3.org/1999/xhtml"  version="2.0"
	...
	<xsl:variable name="g_nstCodeLists" select="document('../../common/codelists.xml')/CodeLists/CodeList"/>
	...
	<xsl:variable name="IsAmendedbyUK" as="xs:boolean" select="exists(/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://www.legislation.gov.uk/def/legislation/EffectsApplied'])"/>
	...
	<xsl:template name="TSOOutputUpdateStatusMessage">
		...
		<xsl:param name="isLargeData" as="xs:boolean" select="false()"/>
		...
		<xsl:variable name="sectionTitle">
			<xsl:choose>
				...
				<xsl:when test="$g_strIntroductionUri = $dcIdentifier">
					<xsl:value-of select="leg:TranslateMessage('Introductory Text')"/>
				</xsl:when>
			</xsl:choose>
		</xsl:variable>
		...
		<div>
			<xsl:choose>
				...
				<xsl:when test="$g_isEUtreaty and not(leg:IsEUPDFOnlyNotRevised(.))">
					...
					<xsl:if test="not(leg:IsCurrentRevised(.))">
						<xsl:attribute name="class">EU</xsl:attribute>
					</xsl:if>
					<h2>
            <xsl:value-of select="leg:TranslateMessage('swhead_status')"/>
          </h2>
					<p class="intro">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:value-of select="concat(leg:TranslateMessage('status_waring_original_version'),'.')"/>
							</xsl:otherwise>
						</xsl:choose>
					</p>
				</xsl:when>
				...
				<xsl:when test="$g_strDocumentMainType = 'EuropeanUnionDirective'  and $g_strDocumentStatus = 'revised'">
					<xsl:attribute name="id">infoSection</xsl:attribute>
					<h2>
            			<xsl:value-of select="leg:TranslateMessage('statuswarning_revised_eudirective_head')"/>
          			</h2>
					<p class="intro">
				...
				<xsl:when test="leg:IsRevisedEUPDFOnly(.) and leg:IsOutstandingEffectExists(.)">
					...
					<div class="title">
						<h2>
						  <xsl:value-of select="leg:TranslateMessage('statuswarning_changes_with_effects_head')"/>
						</h2>
						<p class="intro">
							<xsl:value-of select="leg:TranslateMessage('statuswarning_changes_with_effects_content_p1')"/>
							<xsl:text> </xsl:text>
							<xsl:value-of select="$status-legtitle"/>
							<xsl:text>. </xsl:text>
							<xsl:value-of select="leg:TranslateMessage('statuswarning_changes_with_effects_content_p2')"/>
						</p>
				...
				<xsl:when test="leg:IsRevisedEUPDFOnly(.)">
					<xsl:attribute name="id">statusWarning</xsl:attribute>
					<xsl:choose>
						<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision') and not($pointInTimeView) and not($IsAmendedbyUK)">
							<xsl:attribute name="class">EU-black-border</xsl:attribute>
						</xsl:when>
						<xsl:otherwise>
					...
					<div class="title">
						<h2>
						  <xsl:value-of select="leg:TranslateMessage('swhead_changes_to_legislation')"/>
						</h2>
						<p class="intro">
							<xsl:choose>
								<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision') and not($pointInTimeView) and not($IsAmendedbyUK)">
									<xsl:value-of select="concat(leg:TranslateText('EU_no_outstanding_effects_p1'), ' ')"/>
									<xsl:choose>
										<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation')">
											<xsl:value-of select="leg:TranslateText('Regulation')"/>
										</xsl:when>
										<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionDecision')">
											<xsl:value-of select="leg:TranslateText('Decision')"/>
										</xsl:when>
									</xsl:choose>
									<xsl:value-of select="concat(' ', leg:TranslateText('EU_no_outstanding_effects_p2'), ' ')"/>
									<a href='https://eur-lex.europa.eu/homepage.html' target='_blank'>EUR-Lex</a>
									<xsl:value-of select="concat(' ', leg:TranslateText('EU_no_outstanding_effects_p3'), ' ')"/>
									<a href="{if($TranslateLangPrefix='/cy') then $TranslateLangPrefix else ''}/eu-legislation-and-uk-law" target="_blank">
										<xsl:value-of select="leg:TranslateText('EU_no_outstanding_effects_p4')"/>
									</a>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of select="leg:TranslateMessage('swcontent_tocmsg_eupdf_p1')"/>
									<xsl:text> </xsl:text>
									<xsl:value-of select="$status-legtitle"/>
									<xsl:text>. </xsl:text>
									<xsl:value-of select="leg:TranslateMessage('swcontent_tocmsg_eupdf_p2')"/>
								</xsl:otherwise>
							</xsl:choose>
						</p>
					</div>
				...
				<xsl:when test="leg:IsEUPDFOnlyNotRevised(.) and leg:IsOutstandingEffectExists(.)">
					...
					<div class="title">
						<h2><xsl:value-of select="leg:TranslateMessage('infoSection_head')"/> </h2>
						<p class="intro">
							<xsl:value-of select="leg:TranslateMessage('infoSection_content')"/>
							<xsl:text> </xsl:text>
							<xsl:value-of select="$status-legtitle"/>
							<xsl:text>.</xsl:text>
						</p>
						<p class="c2a">
							<a href="{if($TranslateLangPrefix='/cy') then $TranslateLangPrefix else ''}/changes/unapplied/affected/{$g_strShortType}/{$g_strDocumentYear}/{$g_strDocumentNumber}">
								<xsl:value-of select="leg:TranslateMessage('infoSection_actionlink')"/>
							</a>
						</p>
					</div>
				</xsl:when>
				<xsl:when test="leg:IsEUPDFOnlyNotRevised(.) and not($isLargeData)">
					<xsl:attribute name="id">infoSection</xsl:attribute>
					<xsl:if test="not(leg:IsCurrentRevised(.))">
						<xsl:attribute name="class">EU</xsl:attribute>
					</xsl:if>
					...
					<p class="intro">
						 <xsl:value-of select="leg:TranslateMessage('swcontent_revisedpdf')"/>
					</p>
				</xsl:when>
				<xsl:when test="(not(leg:IsPDFOnlyNotRevised(.)) and ($Scenario = '1' or  $Scenario = '5' or leg:IsCurrentRevised(.)) )">
					<xsl:attribute name="id">statusWarning</xsl:attribute>
					<xsl:attribute name="class">
						<xsl:if test="($Scenario = '1' or ($Scenario = '5' and $IsEditedByEPP and leg:IsOutstandingEffectsOnlyProspectiveOrFutureDate(.))) and empty($g_powerToAmend)">
							<xsl:choose>
								<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision') and not($IsAmendedbyUK) and not($pointInTimeView)">
									<xsl:text>EU-black-border</xsl:text>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>uptoDate</xsl:text>
								</xsl:otherwise>
							</xsl:choose>
						</xsl:if>
					</xsl:attribute>
					...
					<xsl:if test="leg:isFromWestlaw(.)">
						...
						<p class="intro">
						<xsl:value-of select="concat(' ',leg:TranslateMessage('status_warning_revised_is_from_westlaw_pt1'))"/>
						<a href="/contributors#westlaw" class="more"><strong><xsl:value-of select="leg:TranslateMessage('Read more')"/></strong></a>
						</p>
						</div>
					</xsl:if>
					...
					<div class="title">
						...
						<p class="intro">
							...
							<xsl:choose>
								...
								<xsl:when test="leg:IsRevisedPDFOnly(.)">
										<xsl:variable name="reviseddate" as="xs:date" select="max(for $d in ($ndsLegislation/ukm:Metadata/ukm:Alternatives/ukm:Alternative[@Revised castable as xs:date]/@Revised) return xs:date($d))"/>
										<xsl:value-of select="leg:TranslateMessage('swcontent_revisedpdf')"/>
										<xsl:value-of select="concat(' ', leg:TranslateMessage('The electronic revised (latest available) version of the'), ' ')"/>
										<xsl:value-of select="$status-legtitle"/>
										<xsl:value-of select="concat(' ', leg:TranslateMessage('has been created and contributed by the Department for Work and Pensions') , '.')"/>
										<!--<a href="http://www.legislation.gov.uk/contributors#dwp"><strong> Read more.</strong></a>-->
										<xsl:value-of select="concat(' ', leg:TranslateMessage('There may be outstanding changes yet to be applied. This version incorporates changes up to'), ' ')"/>
										<xsl:value-of select="if (exists($reviseddate)) then format-date($reviseddate,'[D]/[M]/[Y]') else 'DD/MM/YYYY'"/>
										<xsl:text>.</xsl:text>
								</xsl:when>
									...
									<xsl:choose>
										...
										<xsl:otherwise>
											<xsl:variable name="title">
												<xsl:choose>
													<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation','EuropeanUnionDecision')">
														<xsl:value-of select="concat(leg:TranslateMessage('no_outstanding_effects_eudn_eudr'), ' ')"/>
													</xsl:when>
													<xsl:otherwise>
														<xsl:value-of select="concat(leg:TranslateMessage('no_outstanding_effects'), ' ')"/>
													</xsl:otherwise>
												</xsl:choose>
												<xsl:value-of select="$status-legtitle"/>
											...
											<xsl:value-of select="replace($title, 'the The', 'The')"/>
										</xsl:otherwise>
									</xsl:choose>
									<xsl:text>.</xsl:text>
								</xsl:when>
								<xsl:when test="$Scenario = '5'">
									<xsl:choose>
										<xsl:when test="$IsEditedByEPP and leg:IsOutstandingEffectsOnlyProspectiveOrFutureDate(.)">
											<xsl:choose>
												...
												<xsl:when test="leg:IsTOC()">
													<xsl:value-of select="$status-legtitle"/>
													<xsl:value-of select="concat(' ', leg:TranslateMessage('status_warning_effects_pt1'), ' ')"/>
													<xsl:call-template name="TranslateLongDate">
														<xsl:with-param name="strInputDate" select="xs:string(current-date())" as="xs:string"/>
													</xsl:call-template>
													<xsl:value-of select="concat('. ', leg:TranslateMessage('status_warning_effects_pt2'), '.')"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="$status-legtitle"/>
													<xsl:if test="string-length($sectionTitle) ne 0">
													...
													<xsl:value-of select="concat('. ', leg:TranslateMessage('status_warning_effects_pt2'), '.')"/>
													<xsl:value-of select="concat(' ', leg:TranslateMessage('status_warning_effects_pt3'), '.')"/>
												</xsl:otherwise>
											</xsl:choose>
										</xsl:when>
										<xsl:when test="leg:IsEURetained(.)">
											<xsl:choose>
												<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision')">
													<xsl:value-of select="concat(leg:TranslateMessage('statuswarning_euretained_p1_eudr_eudn'), ' ')"/>
												</xsl:when>
												<xsl:otherwise>
													<xsl:value-of select="concat(leg:TranslateMessage('statuswarning_euretained_p1'), ' ')"/>
												</xsl:otherwise>
											</xsl:choose>
											<xsl:value-of select="$status-legtitle"/>
											<xsl:text>.</xsl:text>
											<xsl:if test="leg:IsTOC()">
												<xsl:text> </xsl:text>
												<xsl:value-of select="leg:TranslateMessage('status_warning_eu_retained_p_toc')"/>
											</xsl:if>
											<xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('statuswarning_euretained_p2')"/>
										</xsl:when>
										<xsl:otherwise>
											<xsl:value-of select="leg:TranslateMessage('swcontent_tocmsg_p1')"/>
											<xsl:text> </xsl:text>
											<xsl:value-of select="$status-legtitle"/>
											<xsl:text>.</xsl:text>
											...
											<xsl:if test="leg:IsTOC()">
												<xsl:text> </xsl:text>
												<xsl:value-of select="leg:TranslateMessage('swcontent_tocmsg_p2')"/>
											</xsl:if>
											<xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('swcontent_tocmsg_p3')"/>
										</xsl:otherwise>
									</xsl:choose>
								</xsl:when>
						...
						<xsl:sequence select="leg:formatPowerToAmendText($g_powerToAmend)"/>
							...
							<xsl:if test="($Scenario = '5' and not($pointInTimeView and $version != 'prospective'))">
								<xsl:call-template name="processEffects">
									<xsl:with-param name="includeTooltip" select="$includeTooltip" tunnel="yes"/>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="leg:IsDraft(.) and /leg:Legislation/ukm:Metadata/ukm:BillMetadata">
							...
							<h2><xsl:value-of select="leg:TranslateMessage('Draft Bill')" />:</h2>
						</xsl:when>
						<xsl:when test="leg:IsDraft(.)">
							<xsl:attribute name="id">infoDraft</xsl:attribute>
							<h2><xsl:value-of select="leg:TranslateMessage('Draft Legislation')" />:</h2>
						</xsl:when>
						<xsl:when test="leg:IsProposedVersion(.)">
							<xsl:attribute name="id">infoProposed</xsl:attribute>
							<h2><xsl:value-of select="leg:TranslateMessage('Proposed Legislation')" />:</h2>
						</xsl:when>

						<xsl:otherwise>
							<xsl:attribute name="id">infoSection</xsl:attribute>
							<xsl:choose>
								<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision', 'EuropeanUnionDirective', 'EuropeanUnionTreaty') and not(leg:IsCurrentRevised(.))">
									<xsl:attribute name="class">EU</xsl:attribute>
								</xsl:when>
							</xsl:choose>
					...
					<p class="intro">
						<xsl:choose>
							<xsl:when test="(leg:IsDraft(.) or leg:IsProposedVersion(.)) and $g_strDocumentMainType = 'UnitedKingdomDraftPublicBill'" >
								<xsl:text> </xsl:text>
								<xsl:value-of select="leg:TranslateMessage('status_warning_intro_draft_bill_item') " />
							</xsl:when>
							<xsl:when test="leg:IsProposedVersion(.)">
								<strong><xsl:value-of select="/leg:Legislation/ukm:Metadata/dct:alternative" /></strong>
								<xsl:text> </xsl:text>
								<xsl:value-of select="leg:TranslateMessage('status_warning_intro_proposed_version')"/>
							</xsl:when>
							<xsl:when test="leg:IsDraft(.)">
								<xsl:value-of select="leg:TranslateMessage('status_warning_intro_draft_legislation')"/>
								<!--
								...
								<xsl:choose>
									<xsl:when test="exists(/leg:Legislation/ukm:Metadata/ukm:SupersededBy/ukm:Number)">
										<xsl:value-of select="concat('. ',leg:TranslateMessage('This draft has since been made as a'),' ')"/>
										<xsl:value-of select="tso:GetSingularTitleFromType(tso:GetTypeFromDraftType($g_strDocumentMainType,''),'')"/>
										<xsl:text>: </xsl:text>
									</xsl:when>
									<xsl:otherwise>
										<xsl:value-of select="concat(' ',leg:TranslateMessage('and has not yet been made as a'),' ')"/>
										<xsl:value-of select="tso:GetSingularTitleFromType(tso:GetTypeFromDraftType($g_strDocumentMainType,''),'')"/>
										<xsl:text>. </xsl:text>
										<xsl:if test="exists(/leg:Legislation/ukm:Metadata/ukm:SupersededBy)">
											<xsl:value-of select="concat(leg:TranslateMessage('This draft has been replaced by a new draft'),', ')"/>
										</xsl:if>
									</xsl:otherwise>
								</xsl:choose>
								...
								<xsl:if test="leg:IsPDFOnly(.)">
									<xsl:text>. </xsl:text>
									<xsl:value-of select="leg:TranslateMessage('item_available_as_pdf_only')"/>
								</xsl:if>
							</xsl:when>
							<xsl:when test="leg:IsPDFOnly(.) and not($isLargeData)">
								<xsl:value-of select="leg:TranslateMessage('item_available_as_pdf_only')"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:text> </xsl:text>
								<xsl:choose>
									<xsl:when test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision')">
										<span style="font-weight: 500 !important">
											<xsl:value-of select="leg:TranslateText('EU_status_warning_original_version',(concat('type=',leg:GetCodeSchemaStatus(.))))" /></span>
										<br/>
										<xsl:choose>
											<xsl:when test="leg:IsRevisedVersionExists(.)">
												<span>This legislation may since have been updated - </span>
												<span class="active">
													<a href="{leg:FormatURL(//atom:link[@rel='http://purl.org/dc/terms/hasVersion' and @title='current']/@href)}">see the latest available (revised) version</a>
												</span>
											</xsl:when>
											<xsl:otherwise>
												<xsl:if test="$Scenario = '2' or $Scenario = '3'">
													<xsl:value-of select="leg:TranslateMessage(concat('EU_status_warning_original_version',$Scenario))" />
												</xsl:if>
												<xsl:text>.</xsl:text>
											</xsl:otherwise>
										</xsl:choose>
						...
						<xsl:if test="leg:isFromWestlaw(.)">
							<xsl:value-of select="concat(' ',leg:TranslateMessage('status_warning_is_from_westlaw_pt1'))"/>
							<xsl:value-of select="concat(' ',tso:GetSingularTitleFromType($g_strDocumentMainType,''),' ')" />
							<xsl:value-of select="concat(leg:TranslateMessage('status_warning_is_from_westlaw_pt2'),'. ')"/>
							<a href="/contributors#westlaw" class="more"><strong><xsl:value-of select="leg:TranslateMessage('Read more')"/></strong></a>
						</xsl:if>
						<xsl:if test="leg:isFromBritishHistoryOnline(.)">
							<xsl:text> </xsl:text>
							<xsl:value-of select="leg:TranslateText('BritishHistoryOnline')" />
							<br/>
							<a href="/contributors#BritishHistoryOnline" class="more">
								<xsl:text> </xsl:text>
								<strong>
									<xsl:value-of select="leg:TranslateText('Read more')"/>
								</strong>
							</a>
						</xsl:if>
					</p>
		...
		<xsl:if test="$isLargeData">
			<div id="infoSection">
				<xsl:if test="$g_strDocumentMainType = ('EuropeanUnionRegulation', 'EuropeanUnionDecision', 'EuropeanUnionDirective', 'EuropeanUnionTreaty') and not(leg:IsCurrentRevised(.))">
					<xsl:attribute name="class">EU</xsl:attribute>
				</xsl:if>
				<div class="title">
					<h2>Status:</h2>
					<p class="intro">This item of legislation is currently only available to download and view as PDF.  This is an interim measure put in place because the document is too large to open on the website in other formats.</p>
				</div>
			</div>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="TranslateLongDate">
		...
		<xsl:value-of select="string-join(($tokenizedDate[1],leg:TranslateMessage($tokenizedDate[2]),$tokenizedDate[3]),' ')"/>
	</xsl:template>
	...
	<xsl:template name="TSOOutputStatusMessage">
		...
		<xsl:if test="not(leg:IsTOC())">
			...
			<xsl:variable name="pointInTimeText">
				<xsl:choose>
					<xsl:when test="$version = 'prospective'"><xsl:value-of select="leg:TranslateMessage('status_warning_PiT_ prospective')"/>.</xsl:when>
					<xsl:when test="$version castable as xs:date"><xsl:value-of select="concat(leg:TranslateMessage('status_warning_PiT_at'),' ')"/><xsl:value-of select="leg:FormatDate($version)"/>. </xsl:when>
				</xsl:choose>
			</xsl:variable>
            ...
            <xsl:value-of select="leg:TranslateMessage('swhead_status')"/>
						...
						<xsl:if test="$includeTimeline">
							<xsl:choose>
								<xsl:when test="$status/@scenarioId = 'S2' ">
									<xsl:text> </xsl:text>
									<a href="?timeline=true"><xsl:value-of select="leg:TranslateMessage('status_warning_PiT_timeline_changes_p1')"/></a>
								</xsl:when>
								<xsl:when test="$status/@scenarioId = xs:string( if ($pointInTimeView) then ('S9') else ('S6') )">
									<xsl:text> </xsl:text>
									<a href="?timeline=true"><xsl:value-of select="leg:TranslateMessage('status_warning_PiT_timeline_changes_p1')"/></a>
									<xsl:value-of select="concat(' ',leg:TranslateMessage('status_warning_PiT_timeline_changes_p1_1'))"/>
								</xsl:when>
								<xsl:when test="$status/@scenarioId = (if ($pointInTimeView) then 'S12' else 'S9')">
									<xsl:text> </xsl:text>
									<a href="?timeline=true"><xsl:value-of select="leg:TranslateMessage('status_warning_PiT_timeline_changes_p1')"/></a>
									<xsl:value-of select="concat(' ',leg:TranslateMessage('status_warning_PiT_timeline_changes_p1_2'))"/>
								</xsl:when>
							</xsl:choose>
						</xsl:if>
					...
					<xsl:if test="exists($status) and $includeTooltip">
						...
						<xsl:call-template name="TSOOutputStatusHelpTip">
							...
							<xsl:with-param name="helpTitle"><xsl:value-of select="leg:TranslateMessage('Status')"/></xsl:with-param>
						</xsl:call-template>
					</xsl:if>
					<xsl:if test="$status/tso:extent">
						<div class="extentMarkerInfo">
							<h2><xsl:value-of select="leg:TranslateMessage('Skip to')"/>:</h2>
							<ul>
				...
				<div class="content">
					...
					<h3>
						...
						<xsl:choose>
							<xsl:when test="leg:IsRevisedPDFOnly(.)">
									<p>
										<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p1')"/><xsl:text> </xsl:text>
										<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p2')"/>
									</p>
							</xsl:when>
							...
							<p>
								<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p1')"/><xsl:text> </xsl:text>
								<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p3')"/><xsl:text> </xsl:text>
								<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p4')"/>
							</p>
							</xsl:when>
							...
							<xsl:when test="$scenarioId ='5' ">
								<xsl:choose>
									<xsl:when test="$pointInTimeView">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p5')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p6')"/>
										</p>
									</xsl:when>
									<xsl:otherwise>
										<xsl:choose>
											<xsl:when test="leg:IsTOC() and not(leg:IsRevisedEUPDFOnly)">
												<p>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p1')"/><xsl:text> </xsl:text>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p2')"/><xsl:text> </xsl:text>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p7')"/>
												</p>
											</xsl:when>
											<xsl:otherwise>
												<p>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p1')"/><xsl:text> </xsl:text>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p2')"/><xsl:text> </xsl:text>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p8')"/><xsl:text> </xsl:text>
													<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p9')"/>
												</p>
											</xsl:otherwise>
										</xsl:choose>
									</xsl:otherwise>
							...
							<xsl:when test="$scenarioId = '5'  and (leg:IsContent() or leg:IsRevisedEUPDFOnly(.))">
								<p>
									<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p1')"/><xsl:text> </xsl:text>
									<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p2')"/><xsl:text> </xsl:text>
									<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p10')"/><xsl:text> </xsl:text>
								 <xsl:value-of select="leg:TranslateMessage('status_warning_revised_p9')"/>
								</p>
							</xsl:when>
							<xsl:when test="$pointInTimeView">
								<xsl:choose>
									<xsl:when test="$scenarioId = 'S1' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p11')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p12')"/>
										</p>
										<ol>
											<li><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p12i')"/></li>
											<li><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p12ii')"/></li>
										</ol>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p13')"/></p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S2' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p14')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p15')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p16')"/>
										</p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p17')"/></p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S3' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p18')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p19')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p20')"/>
										</p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S4' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p18')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p21')"/>
										</p>
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p20')"/>
										</p>
									...
									<xsl:when test="$scenarioId = 'S5' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p18')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p22')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p23')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p24')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p25')"/>
										</p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S6' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p18')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p22i')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p23')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p24')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p25')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p21i')"/>
										</p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S7' ">
										...
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p26')"/></p><xsl:text> </xsl:text>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p27')"/></p><xsl:text> </xsl:text>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p28')"/></p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S8' ">
										<p>
											...
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p24')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p25')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p29')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p17')"/>
										</p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p28')"/></p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S9' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p18')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p30')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p31')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p32')"/>
										</p>
									</xsl:when>
									...
									<xsl:when test="$scenarioId = 'S11' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p33')"/> </p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S12' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p34')"/> </p>
									</xsl:when>
								</xsl:choose>
							</xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									...
									<xsl:when test="$scenarioId = 'S2' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p14')"/> </p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p35')"/> </p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p16')"/> </p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p17')"/> </p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S3' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p11')"/></p>
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p23')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p24')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p25')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p36')"/>
										</p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S4' ">
										<p>
										...
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p26')"/> </p>
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p27')"/> </p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S5' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p11')"/><xsl:text> </xsl:text>
									...
									<xsl:when test="$scenarioId = 'S6' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p37')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p31')"/>
										</p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S7' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p38')"/> </p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S8' ">
										<p><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p39')"/> </p>
									</xsl:when>
									<xsl:when test="$scenarioId = 'S9' ">
										<p>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p40')"/><xsl:text> </xsl:text>
											<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p31')"/>
										</p>
									</xsl:when>
								</xsl:choose>
	...
	<xsl:template match="leg:P1group" mode="TSOStatusMessage" as="element(tso:status)?">
		<xsl:choose>
			<xsl:when test="$pointInTimeView and
				...
				<tso:status scenarioId="S11"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p41')"/></tso:status>
			</xsl:when>
			<xsl:when test="@Status = 'Prospective'">
				<xsl:choose>
					<xsl:when test="@Concurrent = 'true'">
						<tso:status scenarioId="{if ($pointInTimeView) then 'S8' else 'S5'}">
							<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p42')"/><xsl:text> </xsl:text>
							<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p43')"/>
							<tso:extent><xsl:apply-templates select="." mode="TSOExtentLinks" /></tso:extent>
						</tso:status>
					</xsl:when>
					<xsl:when test="exists(@AltVersionRefs)">
						<tso:status scenarioId="{if ($pointInTimeView) then 'S7' else 'S4'}"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p42')"/></tso:status>
					</xsl:when>
					<xsl:otherwise>
						<tso:status scenarioId="{if ($pointInTimeView) then
								...
								else 'S1'}"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p44')"/> </tso:status>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="@Status = 'Dead'">
				<tso:status scenarioId="{if ($pointInTimeView) then 'S12' else 'S9'}"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p45')"/></tso:status>
			</xsl:when>
			<xsl:when test="@Match = 'false'">
				<xsl:choose>
					<xsl:when test="not($pointInTimeView) and @RestrictStartDate > current-date()">
						<tso:status scenarioId="S7"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p46')"/> <xsl:value-of select="format-date(@RestrictStartDate, '[D01]/[M01]/[Y0001]')" />.</tso:status>
					</xsl:when>
					<xsl:otherwise>
						<tso:status scenarioId="{if ($pointInTimeView) then 'S9' else 'S6'}"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p47')"/> </tso:status>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="@RestrictEndDate">
				<tso:status>
					...
					<xsl:if test="@Concurrent = 'true' and @AltVersionRefs">
						<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p42a')"/>
						<xsl:value-of select="string(' ')"/>
						<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p43')"/>
						<xsl:value-of select="string(' ')"/>
					</xsl:if>
					<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p49')"/> <xsl:value-of select="leg:TranslateMessage('status_warning_revised_p50')"/>
				</tso:status>
			</xsl:when>
			<xsl:when test="@Concurrent = 'true'">
			...
			<xsl:when test="/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion' and @title = 'prospective']">
				<tso:status scenarioId="S2"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p48')"/></tso:status>
			</xsl:when>
		</xsl:choose>
	</xsl:template>
	<xsl:template match="*" mode="TSOStatusMessage" as="element(tso:status)?">
		<xsl:choose>
			<xsl:when test="$pointInTimeView and
					...
					<tso:status scenarioId="S11"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p49')"/> <xsl:apply-templates select="." mode="TSOStatusMessageXXX"/> <xsl:value-of select="leg:TranslateMessage('status_warning_revised_p51')"/></tso:status>
			</xsl:when>
			<xsl:when test="exists(.//*[@Status = 'Prospective'])">
				<tso:status scenarioId="S1"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p49')"/> <xsl:apply-templates select="." mode="TSOStatusMessageXXX"/> <xsl:value-of select="leg:TranslateMessage('status_warning_revised_p52')"/></tso:status>
			</xsl:when>
			<xsl:when test="not($pointInTimeView) and exists(.//*[@Match = 'false' and @RestrictStartDate > current-date()])">
				<tso:status scenarioId="S8"><xsl:value-of select="leg:TranslateMessage('status_warning_revised_p49')"/> <xsl:apply-templates select="." mode="TSOStatusMessageXXX"/> <xsl:value-of select="leg:TranslateMessage('status_warning_revised_p53')"/></tso:status>
			</xsl:when>
			<xsl:when test="exists(.//*[@Match='false'])">
				...
				<xsl:if test="(every $child in (* except (leg:Number, leg:Title))
					...
					<tso:status scenarioId="{if ($pointInTimeView) then 'S9' else 'S6'}">
						<xsl:value-of select="leg:TranslateMessage('status_warning_revised_p49')"/>
						<xsl:apply-templates select="." mode="TSOStatusMessageXXX"/>
						<xsl:value-of select="concat(' ', leg:TranslateMessage('no longer has effect'), '.')"/>
					</tso:status>
				</xsl:if>
				<!--
	...
	<xsl:template match="leg:Part" mode="TSOStatusMessageXXX"><xsl:value-of select="leg:TranslateMessage('part')"/></xsl:template>
	<xsl:template match="leg:Schedule" mode="TSOStatusMessageXXX"><xsl:value-of select="leg:TranslateMessage('schedule')"/></xsl:template>
	<xsl:template match="leg:Pblock" mode="TSOStatusMessageXXX"><xsl:value-of select="leg:TranslateMessage('cross heading')"/></xsl:template>
	<xsl:template match="leg:Chapter" mode="TSOStatusMessageXXX"><xsl:value-of select="leg:TranslateMessage('chapter')"/></xsl:template>
	...
	<xsl:template match="*" mode="TSOExtentLink">
		...
		<a href="#extent-{translate($extent, '+', '-')}">
			<span class="LegExtentRestriction" title="Applies to {tso:extentDescription($extentsToken)}">
				<span class="btr"></span>
				<xsl:value-of select="leg:TranslateExtent($extent)"/>
				<span class="bbl"></span>
				<span class="bbr"></span>
			</span>
			<xsl:text> - </xsl:text>
			 <span class="description"><xsl:value-of select="tso:extentDescription($extentsToken)" /><xsl:value-of select="concat(' ', leg:TranslateMessage('extent'))"/></span>
		</a>
	</xsl:template>
				...
				<xsl:for-each select="ancestor-or-self::*[leg:Pnumber]">
					<xsl:choose>
						<xsl:when test="position() != last()">
							<xsl:value-of select="concat(leg:TranslateMessage('sub'), ' ')"/>
						</xsl:when>
						<xsl:when test="ancestor::*[leg:Schedule]">
							<xsl:value-of select="concat(leg:TranslateMessage('paragraph'), ' ')"/>							
						</xsl:when>
						<xsl:otherwise>
							<xsl:value-of select="concat(leg:TranslateMessage('section'), ' ')"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
			</xsl:if>
	...
	<xsl:template match="leg:Pblock" mode="CurrentSectionName" priority="3">
		<xsl:value-of select="leg:TranslateMessage('Cross Heading')"/><xsl:text>: </xsl:text>
		<xsl:next-match/>
	</xsl:template>
	...
	<!-- this is just checking that a ENGLISH revised is accessible and not 
		specifically checking if the current item is a ENGLISH language item -->	
	<xsl:function name="leg:IsEnglishRevisedVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'revised'
			or $legislation/leg:Legislation/ukm:Metadata/atom:link[not(@hreflang) or @hreflang='en']/@title='current'
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='en'] and ($legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus[@Value = 'revised']))	  
			or $legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/ukm:Alternatives/ukm:Alternative[@Revised]"/>
	</xsl:function>

	<!-- this is just checking that a WELSH revised is accessible and not 
		specifically checking if the current item is a WELSH language item -->
	<xsl:function name="leg:IsWelshRevisedVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'revised'
			or $legislation/leg:Legislation/ukm:Metadata/atom:link[@hreflang='cy' and @title='current']
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='cy'] and ($legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus[@Value = 'revised']))
			or $legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/ukm:Alternatives/ukm:Alternative[@Revised]"/>
	</xsl:function>
	
	<!-- this is just checking that a (WELSH or ENGLISH) revised is accessible and not 
		specifically checking if the current item is a (WELSH or ENGLISH) language item -->
	<xsl:function name="leg:IsRevisedVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>		
		<xsl:sequence select="leg:IsEnglishRevisedVersionExists($legislation) or leg:IsWelshRevisedVersionExists($legislation)"/>
	</xsl:function>


	<xsl:function name="leg:IsEnglishEnactedVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'final'
			or (($legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'final') and ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='en']))
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion' and (@hreflang='en' or empty(@hreflang)) and @title = ('enacted', 'made', 'created', 'adopted')])"/>
	</xsl:function>

	<xsl:function name="leg:IsWelshEnactedVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'final'
			or (($legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'final') and ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='cy']))
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion' and @hreflang='cy' and @title = ('enacted', 'made', 'created', 'adopted')])"/>
	</xsl:function>

	<!-- MR 2025/10/21: new Draft language tests for WDSI -->
	<xsl:function name="leg:IsEnglishDraftVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/ukm:SecondaryMetadata/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'draft'
			or (($legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/ukm:SecondaryMetadata/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'draft') and ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='en']))
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion' and (@hreflang='en' or empty(@hreflang)) and @title = ('draft')])"/>
	</xsl:function>
	
	<xsl:function name="leg:IsWelshDraftVersionExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/ukm:SecondaryMetadata/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'draft'
			or (($legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/ukm:SecondaryMetadata/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'draft') and ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel='alternate' and @hreflang='cy']))
			or ($legislation/leg:Legislation/ukm:Metadata/atom:link[@rel = 'http://purl.org/dc/terms/hasVersion' and @hreflang='cy' and @title = ('draft')])"/>
	</xsl:function>

	<!--[to check if current doc is a ENGLISH revised]-->
	<xsl:function name="leg:IsCurrentEnglishRevised" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'revised'
			or ($legislation/leg:Legislation[(not(@xml:lang) or @xml:lang='en')]/ukm:Metadata/ukm:Alternatives/ukm:Alternative[@Revised] and $paramsDoc/parameters/version/normalize-space(.) = '')"/>
	</xsl:function>
	
	<!--[to check if current doc is a WELSH revised]-->
	<xsl:function name="leg:IsCurrentWelshRevised" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentStatus/@Value = 'revised'
			or ($legislation/leg:Legislation[@xml:lang='cy']/ukm:Metadata/ukm:Alternatives/ukm:Alternative[@Revised] and $paramsDoc/parameters/version/normalize-space(.) = '')"/>
	</xsl:function>
	
	<!--[to check if current doc is a (WELSH or ENGLISH) revised]-->
	<xsl:function name="leg:IsCurrentRevised" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="leg:IsCurrentEnglishRevised($legislation) or leg:IsCurrentWelshRevised($legislation)"/>
	</xsl:function>
	...
	<xsl:function name="leg:IsEnactedExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="leg:IsEnglishEnactedVersionExists($legislation) or leg:IsWelshEnactedVersionExists($legislation)"/>
	</xsl:function>
	...
	<xsl:function name="leg:IsWelshLegType" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="$legislation/leg:Legislation/ukm:Metadata/ukm:*/ukm:DocumentClassification/ukm:DocumentMainType/@Value = ($leg:welshType)" />
	</xsl:function>
	...
	<xsl:function name="leg:isFromBritishHistoryOnline" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()" />
		<xsl:sequence select="$legislation/leg:Legislation/ukm:Metadata/dc:publisher = 'British History Online'" />
	</xsl:function>
	...
	<xsl:function name="leg:IsLegislationWeRevise" as="xs:boolean">
		...
		<xsl:sequence select="$legislation/leg:Legislation/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:DocumentClassification/ukm:DocumentMainType/@Value
			= ('UnitedKingdomPublicGeneralAct', 'UnitedKingdomLocalAct', 'UnitedKingdomPrivateOrPersonalAct', 'GreatBritainPrivateOrPersonalAct', 'GreatBritainLocalAct', 'GreatBritainAct', 'EnglandAct', 'ScottishOldAct', 'ScottishAct',
	...
	<xsl:function name="leg:IsOutstandingEffectExists" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:choose>
			<xsl:when test="leg:IsCurrentWelsh($legislation)">
				<xsl:sequence select="count($legislation/leg:Legislation/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:UnappliedEffects/ukm:UnappliedEffect[not(@RequiresWelshApplied='false')]) > 0"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:function name="leg:IsOutstandingEffectsOnlyProspectiveOrFutureDate" as="xs:boolean">
		...
		<xsl:choose>
			<xsl:when test="leg:IsCurrentWelsh($legislation)">
				<xsl:sequence select="
					every $t in $legislation/leg:Legislation/ukm:Metadata/(ukm:PrimaryMetadata|ukm:SecondaryMetadata|ukm:EUMetadata)/ukm:UnappliedEffects/ukm:UnappliedEffect[not(@RequiresWelshApplied='false')]/ukm:InForceDates/ukm:InForce
	...
	<xsl:function name="leg:is-large-data" as="xs:boolean">
		<xsl:param name="legislation" as="document-node()"/>
		<xsl:sequence select="not(exists($legislation/leg:Legislation/* except $legislation/leg:Legislation/ukm:Metadata)) and exists($legislation/leg:Legislation/processing-instruction('largeData'))"/>
	</xsl:function>
	...
	<xsl:function name="leg:DivisionName" as="xs:string">
		...
		<xsl:value-of select="if ($divelement/@Type = 'EUTitle' and not(matches($divelement/leg:Number, 'title', 'i'))) then leg:TranslateMessage('Title')
			else if ($divelement/@Type = 'EUPart' and not(matches($divelement/leg:Number, 'part', 'i'))) then leg:TranslateMessage('Part')
			else if ($divelement/@Type = 'EUChapter' and not(matches($divelement/leg:Number, 'chapter', 'i'))) then leg:TranslateMessage('Chapter')
			else if ($divelement/@Type = 'EUSection' and not(matches($divelement/leg:Number, 'section', 'i'))) then leg:TranslateMessage('Section')
			else if ($divelement/@Type = 'EUSubsection' and not(matches($divelement/leg:Number, 'section', 'i'))) then leg:TranslateMessage('Sub-Section')
			else if ($divelement/@Type = 'Annotations' and not(matches($divelement/leg:Number, 'annotation', 'i'))) then leg:TranslateMessage('Annotations')
			else if ($divelement/@Type = 'Annotation' and not(matches($divelement/leg:Number, 'annotation', 'i'))) then leg:TranslateMessage('Annotation')
			else leg:TranslateMessage('Division')"/>
	</xsl:function>
		...
		<xsl:value-of>
			<xsl:choose>
				<xsl:when test="$treatyname = 'euratom'">
					<xsl:value-of select="leg:TranslateMessage('treaty_default')"/>
					<xsl:text> </xsl:text>
					<xsl:value-of select="$date"/>
					<xsl:text>. </xsl:text>
					<xsl:value-of select="leg:TranslateMessage('treaty_euratom')"/>
				</xsl:when>
				<xsl:when test="$treatyname = 'teec'">
					...
					<xsl:value-of select="$date"/>
					<xsl:text>. </xsl:text>
					<xsl:value-of select="leg:TranslateMessage('treaty_teec')"/>
				</xsl:when>
				<xsl:when test="$treatyname = 'teu'">
					...
					<xsl:value-of select="$date"/>
					<xsl:text>. </xsl:text>
					<xsl:value-of select="leg:TranslateMessage('treaty_teu')"/>
				</xsl:when>
				<xsl:when test="$treatyname = 'eea-agreement'">
					...
					<xsl:value-of select="$date"/>
					<xsl:text>. </xsl:text>
					<xsl:value-of select="leg:TranslateMessage('treaty_eea')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="leg:TranslateMessage('treaty_default')"/>
					<xsl:text> </xsl:text>
	...
	<xsl:variable name="powerToAmendLinks" as="element()">
		<links>
			...
			<link string="Financial Conduct Authority" link="https://www.handbook.fca.org.uk/instrument"/>
			<link string="Bank of England" link="https://www.bankofengland.co.uk/paper/2019/the-boes-amendments-to-financial-services-legislation-under-the-eu-withdrawal-act-2018"/>
			<link string="Prudential Regulation Authority" link="https://www.bankofengland.co.uk/paper/2019/the-boes-amendments-to-financial-services-legislation-under-the-eu-withdrawal-act-2018"/>
		</links>
	</xsl:variable>
		...
		<xsl:analyze-string select="$string" regex="{string-join($words, '|')}">
			<xsl:matching-substring>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:element name="{$wrapper-name}">
						<xsl:attribute name="href" select="$powerToAmendLinks//*[@string = $current]/@link"/>
