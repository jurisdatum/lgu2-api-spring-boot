	...
	<xsl:variable name="firstTextNode-in-BlockAmdList" as="xs:string">
		<xsl:choose>
			<xsl:when test="ancestor::leg:OrderedList/parent::leg:BlockAmendment">
				<xsl:value-of select="generate-id(ancestor::leg:BlockAmendment[1]/leg:OrderedList[1]/descendant::text()[normalize-space(.) != ''][1])"/>
			</xsl:when>
			<xsl:otherwise>
		...
		<xsl:otherwise>
			<xsl:choose>
				...
				<xsl:otherwise>
					...
					<xsl:if test="name(parent::node())='Title' and name(../..)='P1group' and (ancestor::*[self::leg:BlockAmendment][1][self::leg:BlockAmendment[@TargetClass = 'primary' and not(@Context='schedule')]] or ancestor::leg:P1group/@Layout = 'side')">
