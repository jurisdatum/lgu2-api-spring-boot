...
exclude-result-prefixes="tso atom">
	...
	<xsl:variable name="doc-wsi-2026" as="xs:boolean" select="($g_strDocType = ('WelshStatutoryInstrument', 'WelshDraftStatutoryInstrument') and xs:integer($g_strDocYear) gt 2025 )"/>
	...
	<xsl:variable name="g_strMainFont" as="xs:string">
		<xsl:choose>
			<xsl:when test="$doc-wsi-2026">
				<xsl:text>Arimo</xsl:text>
			</xsl:when>
	...
	<xsl:variable name="g_strDocYear" select="($g_ndsLegMetadata/ukm:Year/@Value)[1]" as="xs:string"/>
...
<!-- Chunyu:Added a condition for versions. We should go from the root call Call HA048969 -->
	...
	<xsl:template match="/">
		...
		<fo:root id="{replace(/(leg:Legislation | leg:EN)/ukm:Metadata/atom:link[@rel = 'self']/@href,'.xml','.pdf')}">
			...
			<xsl:apply-templates select="/leg:Legislation/processing-instruction('largeData')" mode="largedata"/>			
		<xsl:message>||finding  PI</xsl:message>
	...
	<xsl:template match="processing-instruction('largeData')" mode="largedata">
		<fo:page-sequence master-reference="main-sequence" initial-page-number="1" letter-value="auto" format="i" xml:lang="{$g_documentLanguage}">
<xsl:message>||Processing PI</xsl:message>
			...
			<fo:flow flow-name="xsl-region-body" font-family="{$g_strMainFont}" line-height="{$g_strLineHeight}">				
				<fo:block text-align="center">
					<fo:inline font-weight="bold">Large Data Warning:</fo:inline> The stored data is too large to process.
				</fo:block>	
				<xsl:variable name="PDFuri" select='translate(substring-after(., "AltPdf="),"""", "")'/>				
				<fo:block>
					<xsl:attribute name="space-before">24pt</xsl:attribute>
					<xsl:choose>
						<xsl:when test="$PDFuri = 'no alternatives'">
							<xsl:text>There are no alternative PDFs available</xsl:text>
						</xsl:when>
						<xsl:otherwise>
							<fo:block text-align="center">An alternative PDF is available at:</fo:block>
							<fo:block text-align="center" space-before="24pt">
								<fo:basic-link 
									external-destination="url('{$PDFuri}')" 
...
<xsl:template match="leg:P1[not(ancestor::leg:BlockAmendment) and @Match = 'false' and @RestrictEndDate and not(@Status = 'Prospective') and not(ancestor::leg:Contents) and ((($version castable as xs:date) and xs:date(@RestrictEndDate) &lt;= xs:date($version) ) or (not($version castable as xs:date) and xs:date(@RestrictEndDate) &lt;= current-date() ))]//text()" priority="70">
	...
	<xsl:template match="leg:P1group" priority="99">
		<xsl:choose>
			...
			<xsl:otherwise>
				<fo:block space-before="18pt">
					<xsl:if test="ancestor::*[self::leg:BlockAmendment][1][self::leg:BlockAmendment[@Context='schedule']]">
						<xsl:apply-templates select="leg:Title"/>
					</xsl:if>
					<xsl:apply-templates select="leg:P1 | leg:P"/>
				</fo:block>
	...
	<xsl:template match="leg:P1group/leg:Title" priority="1">
		...
		<fo:block font-size="{$g_strBodySize}" font-weight="bold" 
			...
			<xsl:if test="$intHeadingCount ge 6 and ancestor::leg:BlockAmendment[1][self::leg:BlockAmendment[@TargetClass = 'primary' and @Context='schedule']]"><!-- /ukpga/2022/32/section/52 -->
				<xsl:attribute name="font-weight">normal</xsl:attribute>
				<xsl:attribute name="font-style">italic</xsl:attribute>
				<xsl:attribute name="margin-left">34pt</xsl:attribute>
			</xsl:if>
			<xsl:choose>
				<xsl:when test="$g_strDocType = 'ScottishAct'">
	...
	<xsl:template match="leg:P1[not(ancestor::leg:BlockAmendment)]" priority="3">
		<xsl:choose>
			...
			<xsl:otherwise>
				<fo:list-block provisional-label-separation="3pt" provisional-distance-between-starts="39pt" space-before="6pt">		
				...
				<fo:block font-size="{$g_strBodySize}">
					<xsl:attribute name="margin-left">
						<xsl:choose>
							...
							<xsl:when test="ancestor::leg:BlockAmendment[1]/parent::*[self::leg:P3para]">0pt</xsl:when>
			...
			<xsl:otherwise>
				<fo:list-block provisional-label-separation="3pt" provisional-distance-between-starts="30pt" space-before="6pt">		
					...
					<fo:list-item>
						...
						<fo:list-item-body start-indent="body-start()">
							<xsl:if test="parent::leg:P1group and not(preceding-sibling::leg:Title and ancestor::leg:ScheduleBody) and not(preceding-sibling::leg:P1)">
								<xsl:apply-templates select="parent::*/leg:Title"/>
							</xsl:if>
							<fo:block font-size="{$g_strBodySize}" text-align="justify">
								<xsl:apply-templates select="leg:P1para"/>
									...
									<xsl:if test="not(ancestor::*[self::leg:BlockAmendment][1][self::leg:BlockAmendment[@TargetClass = 'primary']] or ancestor::leg:P1group/@Layout = 'side')">
								  		<fo:inline>
											...
											<xsl:apply-templates select="ancestor::leg:P1[1]/leg:Pnumber"/>
								  			<xsl:value-of select="if(string-length(normalize-space((ancestor::leg:P1[1]/leg:Pnumber/@PuncAfter))) gt 0) then ancestor::leg:P1[1]/leg:Pnumber/@PuncAfter else '.'"/>
										</fo:inline>
										<xsl:text>&#8212;</xsl:text>
							  		</xsl:if>
								</xsl:if>
			...
			<xsl:otherwise>
				...
				<xsl:when test="not(preceding-sibling::*[1][self::leg:BlockAmendment])">
					<fo:block>
						...
						<xsl:choose>
							<!-- LEGDEV-3655: ancestor::*[2]/name()='ListItem' - empty ListItem/Para/Text might need closing BlockAmendment quotes, and/or AppendText at the end  -->
							<xsl:when test=". ='' and ancestor::*[2]/name()='ListItem'">
								...
								<xsl:for-each select="ancestor::*[self::leg:BlockAmendment or self::leg:BlockExtract][1]/following-sibling::*">
									<xsl:if test="not($g_strDocClass =  $g_strConstantEuretained)"><xsl:text>&#x201d;</xsl:text></xsl:if>
									<xsl:if test="$processAppendText">
										<xsl:apply-templates select="self::leg:AppendText/node()"/>
									</xsl:if>
								</xsl:for-each>
							</xsl:when>
							<xsl:otherwise>
...
<xsl:template match="leg:Citation[not(@URI)]">
	...
	<xsl:template match="leg:Image">
		...
		<xsl:variable name="strURL" select="replace(//leg:Resource[@id = current()/@ResourceRef]/leg:ExternalVersion/@URI,'http:','https:')" as="xs:string"/>
	...
	<xsl:template match="leg:Group | leg:Part | leg:Chapter | leg:Pblock | leg:PsubBlock | leg:Schedule" name="FuncProcessMajorHeading">
		...
		<!--<xsl:if test="leg:Reference and not(contains($g_strDocType, 'ScottishAct'))">
			<fo:block space-before="18pt" text-align="right">
...
<!-- Suppress repealed content -->
	...
	<xsl:template match="leg:Repeal[@SubstitutionRef]">
		<xsl:param name="showRepeals" select="false()" tunnel="yes" />	
		<xsl:if test="$selectedSectionSubstituted or $showRepeals or @RetainText='true'">
			<xsl:next-match />
		</xsl:if>
	</xsl:template>
		...
		<fo:block>
			<fo:list-block provisional-label-separation="3pt" space-before="{$g_strLargeStandardParaGap}" provisional-distance-between-starts="42pt" text-indent="0pt">	
				<fo:list-item text-indent="0pt">
					...
					<fo:list-item-body start-indent="body-start()">
						<fo:block font-size="{$g_strBodySize}" text-align="justify">
							<xsl:if test="name(leg:Number/following-sibling::*[1]/*[1]) !='OrderedList'">
