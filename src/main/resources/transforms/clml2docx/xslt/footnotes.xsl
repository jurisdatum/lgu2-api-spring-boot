...
<xsl:template name="footnotes">
	...
	<w:footnotes>
		...
		<xsl:apply-templates select="/Legislation//html:td/Footnote[local:footnote-has-ref(.)]" mode="aux">
			<xsl:with-param name="compact-format" select="$compact-format" tunnel="yes" />
		</xsl:apply-templates>
...
<xsl:template match="html:td/Footnote" mode="aux">
	<w:footnote w:id="{ number(substring(@id, 2)) }">
		<xsl:apply-templates />
	</w:footnote>
</xsl:template>
...
<xsl:template match="FootnoteText/*[1]/*[1][self::Text] | FootnoteText/*[1][self::Text]" priority="1">
    ...
    <w:p>
		...
		<xsl:choose>
			...
			<xsl:when test="not(local:footnote-has-ref($footnote)) and empty($footnote/parent::Footnotes)">	<!-- it's a table (foot)note --> <!-- nisr/2002/20/made -->
		        <w:r>
		            <w:rPr>
		                <w:rStyle w:val="FootnoteReference"/>
		            </w:rPr>
