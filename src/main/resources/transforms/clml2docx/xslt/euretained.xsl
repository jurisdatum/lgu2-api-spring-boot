...
<xsl:template match="EUPreamble/CommentaryRef">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<xsl:choose>
				<xsl:when test="exists(following-sibling::*[last()][self::P/child::Text])" />
				<xsl:otherwise>
					<xsl:message terminate="{ $debug }">CommentaryRef will be missed</xsl:message>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:otherwise>
	</xsl:choose>
...
<xsl:template match="EUPreamble/*[last()]/self::P/Text[last()]" priority="10">
	<xsl:call-template name="p">
		<xsl:with-param name="style" select="'EUPreamble'" />
		<xsl:with-param name="add-ending-runs" as="element()*">
...
<xsl:template name="big-level-eu">
	<xsl:param name="outline-level" as="xs:integer" select="-1" tunnel="yes" />
	<xsl:apply-templates select="Number | Title" mode="eu" />
	<xsl:apply-templates select="* except (Number, Title)" mode="eu">
...
<xsl:template match="EUPart | EUTitle | EUChapter | EUSection | EUSubsection" mode="eu">
	<xsl:call-template name="big-level-eu" />
</xsl:template>
