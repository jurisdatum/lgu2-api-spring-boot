...
<xsl:template match="Tabular/Subtable">
	<w:p>
		<xsl:next-match />
	</w:p>
</xsl:template>
...
<xsl:template match="html:tfoot/html:tr[every $n in html:td/node() satisfies $n/self::Footnote[local:footnote-has-ref(.)]]" />

<xsl:template match="html:tfoot/html:tr/html:td[every $n in node() satisfies $n/self::Footnote[local:footnote-has-ref(.)]]" />

<xsl:template match="html:td/Footnote">
	<xsl:choose>
		<xsl:when test="local:footnote-has-ref(.)" />
		<xsl:otherwise>
			<xsl:apply-templates />
		</xsl:otherwise>
	</xsl:choose>
