	...
	<xsl:choose>
		<xsl:when test="local:show-commentary-marker($type)">
			<w:r>
				...
				<xsl:if test="$is-first-ref-to-commentary and empty(ancestor::Footnote[local:footnote-has-ref(.)])">
...
<xsl:template match="Addition | Substitution | Repeal">
	...
	<!-- LEGDEV-7201 MR 2024/07/29: Addition/@CommentaryRef in ukpga/2000/17/2023-06-01 has same value as its child CommentaryRef/@Ref value which caused docx error with duplicate footnote refs. $is-duplicate-commentaryref tests for this -->
	<xsl:variable name="is-duplicate-commentaryref" as="xs:boolean" select="if (child::CommentaryRef[@Ref = current()/@CommentaryRef]) then true() else false()"/> 
		...
		<xsl:if test="exists($commentary-number)">
			<w:r>
			    ...
			    <xsl:if test="$is-first-ref-to-commentary and empty(ancestor::Footnote[local:footnote-has-ref(.)])">
			    	...
			    	<!-- LEGDEV-7201 MR 2024/07/29: only output if boolean is not true -->
			    	<xsl:if test="exists($footnote-id) and not($is-duplicate-commentaryref)">
					    <w:footnoteReference w:id="{ $footnote-id }" w:customMarkFollows="true" />
			    	</xsl:if>
			    </xsl:if>
		        <w:t>
