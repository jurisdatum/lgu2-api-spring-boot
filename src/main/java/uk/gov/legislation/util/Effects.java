...
public class Effects {
    ...
    static Stream<RichTextNode> flatten1(RichTextNode clml) {
        if (clml instanceof RichTextNode.Text text) return Stream.of(text);
        if (clml instanceof RichTextNode.Section section) return Stream.of(section);
        if (clml instanceof RichTextNode.Range range) return flatten(range.children);
    ...
    static boolean relatesTo(Effect effect, Set<String> ids, boolean includeWhole) {
        ...
        if (sections.isEmpty()) return includeWhole;
        if (sections.stream().anyMatch(n -> ids.contains(n.ref))) return true;
     ...
     */
    public static List<Effect> removeThoseWithNoRelevantSection(
            List<Effect> effects, Set<String> ids, boolean includeWhole) {
