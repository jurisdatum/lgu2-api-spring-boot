...
public class Links {
    ...
    public static Components parse(String link) {
        ...
        if (link.startsWith("http://www.legislation.gov.uk/")) link = link.substring(30);
        else if (link.startsWith("https://www.legislation.gov.uk/")) link = link.substring(31);
        //        else
        //            return null;
        if (link.startsWith("id/")) link = link.substring(3);

        Matcher matcher = Pattern.compile("^([a-z]{3,5})/(\\d{4})/(\\d+)/?").matcher(link);
        if (!matcher.find()) {
            matcher =
                    Pattern.compile("^([a-z]{3,5})/([A-Za-z0-9]+/[0-9-]+)/(\\d+)/?").matcher(link);
        ...
        if (split.length == 0) throw new IllegalStateException(link);
        ...
        if ("english".equalsIgnoreCase(split[split.length - 1])
                || "welsh".equalsIgnoreCase(split[split.length - 1])) {
        ...
        if (split.length == 0)
            return new Components(
                    type, year, number, Optional.empty(), false, Optional.empty(), language);
        ...
        if (split.length == 0)
            return new Components(
                    type, year, number, Optional.empty(), isContents, version, language);
