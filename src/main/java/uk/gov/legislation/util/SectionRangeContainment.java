...
/**
 * Determines whether a provision identifier falls within a range defined by start and end provision
 * identifiers.
 *
 * <p>Provision identifiers are hyphen-separated tokens such as "section-1",
 * "schedule-14-paragraph-3", or "section-395-13-bc". A range like "section-249-1" to
 * "section-249-3" includes "section-249-2".
 *
 * <p>Handles hierarchical containment in both directions:
 *
 * <ul>
 *   <li>"part-2-section-5" is within range "part-1" to "part-3" (descendant)
 *   <li>"section-1" contains range "section-1-a" to "section-1-c" (ancestor)
 * </ul>
 *
 * <p>Uses a top-down approach: strips matching prefix tokens, then checks containment of the
 * remaining tokens. This naturally handles the ancestor case (id exhausted after prefix stripping)
 * and the descendant case (id truncated to range depth before comparison).
 *
 * <p>Ported from the legacy MarkLogic XQuery function utils:contains-section, with improvements for
 * hierarchical containment, alphabetic tokens, and roman numeral handling.
 */
class SectionRangeContainment {
    ...
    static boolean contains(String start, String end, String id) {
        ...
        if (idTokens.length > rangeDepth) idTokens = Arrays.copyOf(idTokens, rangeDepth);
    ...
    /**
     * Compares two provision identifiers. A shorter ID sorts before a longer one when all leading
     * tokens match (e.g. "part-2" &lt; "part-2-section-5").
     */
    ...
    private static final Pattern PROVISION_NUMBER =
            Pattern.compile("^([a-zA-Z]*)(\\d+)([a-zA-Z0-9]*)$");
    private static final Pattern ROMAN_CHARS =
            Pattern.compile("^[ivxlcdm]+$", Pattern.CASE_INSENSITIVE);
    ...
    private static int compareTokens(String a, String b) {
        ...
        if (ma.matches() && mb.matches()) {
            ...
            if (catA == 0) return compareSuffix(ma.group(1), mb.group(1));
            if (catA == 2) return compareSuffix(ma.group(3), mb.group(3));
            return 0;
        }
        ...
        if (ROMAN_CHARS.matcher(a).matches()
                && ROMAN_CHARS.matcher(b).matches()
                && (a.length() > 1 || b.length() > 1)) {
            int ra = Roman.parse(a);
            int rb = Roman.parse(b);
            if (ra > 0 && rb > 0) return Integer.compare(ra, rb);
        }
     ...
     *
     * <ul>
     *   <li>Z followed by a letter is a prefix meaning "before" (ZA, ZB... sort before A)
     *   <li>Letters A through Y are ordered normally
     *   <li>Z at the end (or followed by a digit) is the letter Z (sorts after Y)
     *   <li>The convention is recursive: ZZA sorts before ZA, which sorts before A
     * </ul>
     *
     * <p>Example ordering: ZZA &lt; ZA &lt; ZB &lt; A &lt; AZA &lt; AA &lt; AB &lt; AZ &lt; B &lt;
     * ... &lt; Z
     */
    ...
    /**
     * Returns the sort rank of the first character of a suffix string. Z followed by a letter = 0
     * (before everything). A through Y = 1 through 25. Z at end or followed by a digit = 26 (after
     * Y).
     */
    private static int suffixRank(String s) {
        char first = Character.toUpperCase(s.charAt(0));
        if (first == 'Z' && s.length() > 1 && Character.isLetter(s.charAt(1))) return 0;
        return first - 'A' + 1;
    }
}
