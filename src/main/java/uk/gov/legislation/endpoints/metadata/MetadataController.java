...
public class MetadataController implements MetadataApi {
    ...
    public MetadataController(
            Legislation marklogic, Simplify simplify, UnappliedEffectsFetcher effectsFetcher) {
    ...
    @Override
    public ResponseEntity<String> xml(String type, String year, int number, Locale locale)
            throws Exception {
        validateType(type);
    ...
    @Override
    public ResponseEntity<ExtendedMetadata> json(
            String type, String year, int number, Locale locale) throws Exception {
        validateType(type);
        String language = locale.getLanguage();
        Legislation.Response leg = marklogic.getMetadata(type, year, number, Optional.of(language));
