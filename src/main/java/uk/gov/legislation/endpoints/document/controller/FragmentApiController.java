...
public class FragmentApiController implements FragmentApi {
     ...
     * @throws IOException             if there is an issue with document retrieval
     * @throws InterruptedException    if the operation is interrupted
     * @throws NoDocumentException     if the document does not exist
     */
    @Override
    public ResponseEntity<String> getFragmentClml(String type, int year, int number, String section, Optional<String> version) throws IOException, InterruptedException, NoDocumentException {

        return fragmentService.getDocumentSection(type, year, number, section, version)
                .map(clml -> ResponseEntity.ok().contentType(MediaType.APPLICATION_XML).body(clml))
        ...
        return fragmentService.getDocumentSection(type, year, number, section, version)
                .map(clml -> {
                    try {
                        ...
                        throw new TransformationException(TRANSFORMATION_FAIL,e);
                    }
                })
                .map(akn -> ResponseEntity.ok().contentType(MediaType.parseMediaType("application/akn+xml")).body(akn))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
    }
