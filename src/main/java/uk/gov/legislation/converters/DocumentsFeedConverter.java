...
public class DocumentsFeedConverter {
    ...
    private static PageOfDocuments.Counts convertCounts(
            SearchResults.Facets facets, Integer total) {
        ...
        if (total != null) counts.total = total;
            ...
            counts.total = facets.facetYears.entries.stream().mapToInt(facet -> facet.total).sum();
    ...
    private static SortedSet<String> convertSubjects(SearchResults.Subjects subjects) {
        if (subjects == null) return Collections.emptySortedSet();
        if (subjects.headings == null) return Collections.emptySortedSet();
        return subjects.headings.stream()
                .map(heading -> heading.name)
                .collect(Collectors.toCollection(TreeSet::new));
    }

    private static List<PageOfDocuments.Document> convertDocuments(
            List<SearchResults.Entry> entries) {
        if (entries == null) return Collections.emptyList();
        return entries.stream().map(DocumentsFeedConverter::convertDocument).toList();
    }

    private static final Pattern IA =
            Pattern.compile(
                    "https?://www\\.legislation\\.gov\\.uk/([a-z0-9/-]+)/impacts/\\d{4}/\\d+");
    ...
    private static PageOfDocuments.Document convertDocument(SearchResults.Entry entry) {
        ...
        else doc.id = entry.id;
        doc.longType = entry.mainType == null ? null : entry.mainType.value;
        doc.year = entry.year == null ? 0 : entry.year.value;
        if (entry.number != null) doc.number = entry.number.value;
        if (entry.isbn != null) doc.isbn = entry.isbn.value;
        doc.altNumbers = convertAltNumbers(entry.altNumbers);
        if (doc.number != null)
            doc.cite =
                    Cites.convertNumbersAndMake(doc.longType, doc.year, doc.number, doc.altNumbers);
        else if (doc.isbn != null) doc.cite = "ISBN " + ISBN.format(doc.isbn);
    ...
    private static List<String> getSubjects(String longType, SearchResults.Entry entry) {
        ...
        if (entry.subjects == null) return List.of();
        return entry.subjects.stream().map(s -> s.value).toList();
    }

    /* alt numbers */

    private static List<CommonMetadata.AltNumber> convertAltNumbers(
            List<SearchResults.Entry.AlternativeNumber> altNumbers) {
        if (altNumbers == null) return Collections.emptyList();
        return altNumbers.stream().map(DocumentsFeedConverter::convertAltNumber).toList();
    }

    private static CommonMetadata.AltNumber convertAltNumber(
            SearchResults.Entry.AlternativeNumber atom) {
