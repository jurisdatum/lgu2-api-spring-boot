...
public class Metadata {
    ...
    public Optional<LocalDate> getPointInTime() {
        if (FINAL.equals(status)) return Optional.empty();
        Links.Components comps = Links.parse(dcIdentifier);
        if (comps == null) return Optional.empty();
        if (comps.version().isEmpty()) return Optional.empty();
        LocalDate date;
        try {
     ...
     * milestone when the fragment is served from a later revised representation.
     */
    private LocalDate valid;

    @JsonSetter("valid")
    public void setValid(String value) {
        valid = LocalDate.parse(value);
    }
     ...
     * label-only {@code prospective} value that mirrors legislation.gov.uk's timeline label.
     */
    public String version() {
        if (_computedVersion != null) return _computedVersion;
        _computedVersion = computeVersion();
        return _computedVersion;
    }

    private String computeVersion() {
        if (isProspectiveRevised()) return PROSPECTIVE;
        if (valid == null) return FirstVersion.getFirstVersion(longType);
     ...
     * value left in {@code selected} at the end is the latest eligible milestone.
     */
    ...
    private String latestEligibleMilestoneNotAfter(LocalDate dctValid) {
        ...
        for (String label : versions()) {
            ...
            try {
                ...
                if (!milestone.isAfter(dctValid)) selected = label;
    ...
    /**
     * A single {@code <hasVersion>} entry, carrying the source link's {@code hreflang} attribute so
     * language-aware filtering can tell Welsh links apart from English ones in bilingual XML.
     */
    public static class HasVersionEntry {

        /**
         * BCP 47 language tag of the source {@code atom:link} ({@code "en"} or {@code "cy"}), or
         * {@code null} when the link carries no {@code @hreflang}. Untagged entries apply to both
         * languages.
         */
        @JacksonXmlProperty(isAttribute = true)
        ...
        /**
         * The milestone label from the source link's {@code @title}. Either an ISO date ({@code
         * "2024-11-22"}), a first-version keyword ({@code "enacted"}, {@code "made"}, {@code
         * "created"}, {@code "adopted"}), the {@code "current"} alias for the latest snapshot, a
         * literal {@code "prospective"}, or a dated-and-repealed label ({@code "2020-12-31
         * repealed"}) which is normalised back to its base date by {@link #versions()}.
         */
        @JacksonXmlText public String title;
    ...
    @JsonSetter
    public void setVersions(List<HasVersionEntry> value) {
        _versions = value;
    }
     ...
     * narrowed to links for the response language, plus untagged links. The returned {@code
     * dct:valid} date is not automatically in this scope; for a fragment it may identify a
     * containing-document snapshot rather than a fragment milestone.
     *
     ...
     * that the API still needs to expose.
     *
     * <ol>
     *   <li>Keep {@code hasVersion} entries whose {@code hreflang} matches the response language,
     *       plus any untagged entries.
     *   <li>Strip the trailing {@code " repealed"} suffix.
     *   <li>Remove the {@code current} alias; remember whether it was present and whether it was
     *       the only retained link.
     *   <li>For {@code final} status, ensure the type-specific first-version keyword is present.
     *   <li>For {@code final} XML where {@code current} was the only retained entry, synthesise a
     *       label-only {@code prospective} entry.
     *   <li>For prospective revised content, synthesise a label-only {@code prospective} entry.
     ...
     *       responses, and for fragment responses only when no other dated fragment labels remain.
     * </ol>
     */
    public SortedSet<String> versions() {
        if (_computedVersions != null) return _computedVersions;
        if (_versions == null) _versions = List.of();
        TreeSet<String> result = new TreeSet<>(Versions.COMPARATOR);
        for (HasVersionEntry entry : _versions) {
            if (!matchesResponseLanguage(entry.hreflang)) continue;
        ...
        if (isFinal) result.add(FirstVersion.getFirstVersion(longType));
        if (isFinal && onlyCurrent) result.add(PROSPECTIVE);
        if (isProspectiveRevised()) result.add(PROSPECTIVE);
     ...
     *
     * <p>Stripping the {@code current} alias can leave the result set with no label identifying the
     * returned representation. {@code dct:valid} is the valid-from date of that representation, so
     * using it as a label recovers the identity that was lost — but only when {@code current} was
     * actually present in the scoped input; otherwise there is nothing to recover.
     *
     * <p>The whole-document vs fragment asymmetry: a whole-document response's {@code dct:valid}
     * identifies the returned document snapshot, so when {@code current} was stripped we recover it
     * as the missing document label. A fragment response's {@code dct:valid} may be a
     ...
     * fragment-specific milestones are unavailable.
     */
    ...
    private static boolean hasDatedLabel(SortedSet<String> labels) {
        for (String s : labels) if (Versions.isDateLabel(s)) return true;
        return false;
    }
    ...
    private static String stripRepealed(String title) {
        return title.endsWith(REPEALED)
                ? title.substring(0, title.length() - REPEALED.length())
                : title;
    }

    @JacksonXmlProperty public HasParts hasParts;

    public static class HasParts {

        @JacksonXmlProperty public String introduction;

        @JacksonXmlProperty public String signature;

        @JacksonXmlProperty public String schedules;

        @JacksonXmlProperty public String note;

        @JacksonXmlProperty public String earlierOrders;
    }

    @Deprecated public boolean schedules;

    @JsonSetter("schedules")
    public void setSchedules(String value) {
        schedules = value != null && !value.isBlank();
    }
    ...
    public List<String> formats() {
        return _formats.stream().map(f -> f.name).toList();
    }
    ...
    /**
     * Raw label string from the Atom {@code link[@rel='prev']/@title} attribute. The MarkLogic
     * script generates this as a semicolon‑separated list of components (e.g. "Provision; Section
     * 2"). The converter uses only the first component as the API label for now.
     */
    ...
    /**
     * Raw label string from the Atom {@code link[@rel='next']/@title} attribute. Like {@link
     * #prevTitle}, this is semicolon‑separated; only the first component is exposed by the API at
     * present.
     */
    @JacksonXmlProperty public String nextTitle;
     ...
     *
     * <p>Set by {@code simplify/metadata.xsl} from the target's {@code @Status} attribute — the
     * root {@code <Legislation>} for whole-document requests, or the fragment element for fragment
     * requests. Drives the "add {@code prospective} to versions" rule for prospective revised
     * content.
     */
    @JacksonXmlProperty
    ...
    @JacksonXmlElementWrapper(
            localName = "UnappliedEffects",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "UnappliedEffect",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Effect> rawEffects = Collections.emptyList();

    /**
     * Set to true when unapplied effects have been successfully fetched from MarkLogic for final
     * documents.
     */
    @JsonIgnore public boolean finalEffectsEnriched = false;

    /* ConfersPower and BlanketAmendment */

    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(
            localName = "ConfersPower",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<TitledThing> confersPower;

    @JacksonXmlElementWrapper(useWrapping = false)
    @JacksonXmlProperty(
            localName = "BlanketAmendment",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    ...
    @JacksonXmlElementWrapper(
            localName = "PolicyEqualityStatements",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "PolicyEqualityStatement",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    ...
    @JacksonXmlElementWrapper(
            localName = "CorrectionSlips",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "CorrectionSlip",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> correctionSlips;

    @JacksonXmlElementWrapper(
            localName = "CodesOfPractice",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "CodeOfPractice",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> codesOfPractice;

    @JacksonXmlElementWrapper(
            localName = "CodesOfConduct",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "CodeOfConduct",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> codesOfConduct;

    @JacksonXmlElementWrapper(
            localName = "TablesOfOrigins",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "TableOfOrigins",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> tablesOfOrigins;

    @JacksonXmlElementWrapper(
            localName = "TablesOfDestinations",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "TableOfDestinations",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> tablesOfDestinations;

    @JacksonXmlElementWrapper(
            localName = "OrdersInCouncil",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "OrderInCouncil",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> ordersInCouncil;

    @JacksonXmlElementWrapper(
            localName = "ImpactAssessments",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "ImpactAssessment",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<ImpactAssessment> impactAssessments;

    @JacksonXmlElementWrapper(
            localName = "OtherDocuments",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "OtherDocument",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> otherDocuments;

    @JacksonXmlElementWrapper(
            localName = "ExplanatoryDocuments",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "ExplanatoryDocuments",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    ...
    @JacksonXmlElementWrapper(
            localName = "TranspositionNotes",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "TranspositionNote",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    public List<Alternative> transpositionNotes;

    @JacksonXmlElementWrapper(
            localName = "UKRPCOpinions",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
    @JacksonXmlProperty(
            localName = "UKRPCOpinion",
            namespace = "http://www.legislation.gov.uk/namespaces/metadata")
