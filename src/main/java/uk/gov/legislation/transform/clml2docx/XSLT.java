...
public class XSLT {
    ...
    XSLT(Delegate delegate) throws IOException {
        ...
        processor
                ...
                .registerExtensionFunction(new SaxonExtensionFunctions.GetImageWidth(delegate));
        processor
                .getUnderlyingConfiguration()
                .registerExtensionFunction(new SaxonExtensionFunctions.GetImageHeight(delegate));
        processor
                .getUnderlyingConfiguration()
                .registerExtensionFunction(new SaxonExtensionFunctions.GetImageType(delegate));
        ...
        try {
            docType =
                    xPathCompiler.compile(
                            "/Legislation/ukm:Metadata/*/ukm:DocumentClassification/ukm:DocumentMainType/@Value");
            resourceURIs =
                    xPathCompiler.compile("/Legislation/Resources/Resource/ExternalVersion/@URI");
        } catch (SaxonApiException e) {
            throw new RuntimeException(e);
        }
    }
     ...
     * @return The document that was produced
     */
    private XdmNode callTemplate(
            XdmNode clml, String template, Map<String, Resource> cache, boolean debug) {
