...
public class RegnalYearQuery {
    ...
    private static String constructQueryForRegnal(String regnal, Integer regnalYear) {
        return makeSingleConstructQuery(
                String.format("http://www.legislation.gov.uk/id/regnal/%s/%s", regnal, regnalYear));
    }
    ...
    public Optional<RegnalYearLD> fetchMappedData(String reign, Integer regnalYear)
            throws IOException, InterruptedException {
        ArrayNode graphNodes =
                Graph.extract(fetchRawData(reign, regnalYear, "application/ld+json"));
