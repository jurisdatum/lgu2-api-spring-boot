...
public class RdfMapper {
    ...
    public <T> T read(Map<URI, List<TypedValue>> properties, Class<T> clazz) {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            try {
                if (field.getType().equals(String.class)) field.set(t, values.getFirst().value());
                else if (field.getType().equals(int.class))
                    field.setInt(t, Integer.parseInt(values.getFirst().value()));
                else if (field.getType().equals(List.class)) {
                    //                    Type t2 = ((ParameterizedType)
                    // field.getGenericType()).getActualTypeArguments()[0];
                    //                    if (t2.equals(String.class))
                    field.set(
                            t, values.stream().map(TypedValue::value).collect(Collectors.toList()));
