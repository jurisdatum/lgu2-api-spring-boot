...
/**
 * Synthetic wsi/2020/1609/part/3/chapter/1 fragment with bilingual hasVersion links, built by
 * mutating the committed fragment_wsi_2024_1002_regulation_2_cy.xml fixture.
 *
 * <p>Welsh-side links (hreflang="cy") stop at 2021-02-27; English-side links (hreflang="en") extend
 * to 2022-03-28. The pair exists to exercise language-aware hasVersion filtering in both
 * directions.
 */
public final class BilingualFragmentFixtures {
    ...
    private static String build(Side side, String identifierSuffix, boolean includeCurrentLinks)
            throws IOException {
        // Welsh translation lags English, so the Welsh-side dct:valid is earlier than the English
        // one.
        ...
        clml =
                requireReplace(
                        ...
                        "title=\"HTML5 snippet\"/>",
                        "title=\"HTML5 snippet\"/>\n"
                                + BILINGUAL_HAS_VERSION_LINKS
                                + (includeCurrentLinks ? CURRENT_HAS_VERSION_LINKS : ""));

        clml =
                requireReplace(
                        clml,
                        "http://www.legislation.gov.uk/wsi/2024/1002/regulation/2/made/welsh",
                        "http://www.legislation.gov.uk/wsi/2020/1609/part/3/chapter/1"
                                + identifierSuffix);

        clml =
                requireReplace(
                        clml,
                        "<dc:modified>2024-10-09</dc:modified><dc:subject"
                                + " scheme=\"SIheading\">ADDYSG, CYMRU</dc:subject>",
                        "<dc:modified>2024-10-09</dc:modified><dct:valid>"
                                + validDate
                                + "</dct:valid><dc:subject scheme=\"SIheading\">ADDYSG,"
                                + " CYMRU</dc:subject>");
        ...
        if (side == Side.ENGLISH) {
            clml =
                    requireReplace(
                            clml, "<dc:language>cy</dc:language>", "<dc:language>en</dc:language>");
        }
    ...
    private static String requireReplace(String source, String target, String replacement) {
        if (!source.contains(target))
            throw new IllegalStateException(
                    "Base fixture no longer contains expected anchor: " + target);
