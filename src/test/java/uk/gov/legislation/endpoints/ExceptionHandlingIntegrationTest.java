...
/**
 * Integration tests for exception handling across the application. Tests the integration between
 * controllers and GlobalExceptionHandler.
 */
@SpringBootTest
...
class ExceptionHandlingIntegrationTest {
    ...
    void shouldReturn404_whenDocumentNotFound() throws Exception {
        ...
        mockMvc.perform(get("/document/ukpga/2020/1").accept(MediaType.APPLICATION_JSON))
    ...
    void shouldReturn400_whenInvalidDocumentType() throws Exception {
        mockMvc.perform(get("/document/invalid-type/2020/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Unknown Document Type Error"))
                .andExpect(
                        jsonPath("$.message")
                                .value("The document type 'invalid-type' is not recognized."));
    }
    ...
    void shouldReturn404_whenFragmentNotFound() throws Exception {
        ...
        when(marklogic.getDocumentSection(
                        anyString(), anyString(), anyInt(), anyString(), any(), any()))
                .thenThrow(new NoDocumentException(error));

        mockMvc.perform(get("/fragment/ukpga/2020/1/section-1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").exists())
                .andExpect(jsonPath("$.message").exists());
    }
    ...
    void shouldReturn400_whenInvalidFragmentType() throws Exception {
        mockMvc.perform(
                        get("/fragment/invalid-type/2020/1/section-1")
    ...
    void shouldReturn404_whenContentsNotFound() throws Exception {
        ...
        mockMvc.perform(get("/contents/ukpga/2020/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.error").exists())
                .andExpect(jsonPath("$.message").exists());
    }
    ...
    void shouldReturn400_whenInvalidContentsType() throws Exception {
        mockMvc.perform(get("/contents/invalid-type/2020/1").accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.error").value("Unknown Document Type Error"))
                .andExpect(
                        jsonPath("$.message")
