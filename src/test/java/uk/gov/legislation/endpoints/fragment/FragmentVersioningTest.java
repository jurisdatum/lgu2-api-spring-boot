...
class FragmentVersioningTest {
    ...
    void currentUnversionedFragment_usesLastFragmentMilestoneAsVersion() throws Exception {
        ...
        when(marklogic.getDocumentSection(
                        "ukpga", "2000", 8, "section-91", Optional.empty(), Optional.of("en")))
                .thenReturn(response);

        mockMvc.perform(
                        get("/fragment/ukpga/2000/8/section-91")
    ...
    void welshFragmentVersions_areLanguageAware() throws Exception {
        Legislation.Response response =
                new Legislation.Response(BilingualFragmentFixtures.welsh(), Optional.empty());
        when(marklogic.getDocumentSection(
                        "wsi",
                        "2020",
                        1609,
        ...
        mockMvc.perform(
                        get("/fragment/wsi/2020/1609/part-3-chapter-1")
                                .accept(MediaType.APPLICATION_JSON)
    ...
    void versionedWelshFragment_pointInTimeDoesNotOverrideWelshFragmentVersion() throws Exception {
        Legislation.Response response =
                new Legislation.Response(
                        BilingualFragmentFixtures.welshPointInTime(), Optional.empty());
        when(marklogic.getDocumentSection(
                        "wsi",
                        "2020",
                        1609,
                        "part-3-chapter-1",
                        Optional.of("2021-05-17"),
                        Optional.of("cy")))
                .thenReturn(response);
    ...
    void englishFragmentVersions_areLanguageAware() throws Exception {
        Legislation.Response response =
                new Legislation.Response(BilingualFragmentFixtures.english(), Optional.empty());
        when(marklogic.getDocumentSection(
                        "wsi",
                        "2020",
                        1609,
        ...
        mockMvc.perform(
                ...
                .andExpect(
                        jsonPath(
                                "$.meta.versions",
                                contains("made", "2021-05-17", "2021-08-07", "2022-03-28")))
                .andExpect(
                        jsonPath(
                                "$.meta.versions",
                                not(
